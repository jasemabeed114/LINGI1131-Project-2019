...
Dans cette partie du projet, nous avons mis en place 2 joueurs. Un joueur simple qui agit de manière assez aléatoire et un autre joueur plus intelligent qui agit de manière statégique. De plus, lorsque les extentions sont activées, les joueurs peuvent rester sur place s'ils le désirent.
...
Étant donné que le gagnant est le joueur possédant le plus de points à la fin de la partie, la stratégie de notre joueur avancé est de se concentrer sur les boîtes de points/bonus et non sur l'adversaire. Le joueur ne prête donc aucun intérêt aux joueurs adverses, mais uniquement à sa sécurité et aux points potentiels de la carte de jeu. Nous pouvons voir le comportement de notre joueur comme une suite de vérifications "d'état" pour le prochain mouvement à faire:
\begin{itemize}
...
Etant donné que le gagnant est le joueur possèdant le plus de point à la fin du jeu. La stratégie  de ce joueur sera, par conséquant, d'engranger le plus de point possible. \\
En premier lieu, il posera une bombe s'il possède au moins une bombe et qu'il soit à côté d'une case ayant une \emph{box} non cassé. En effet, la bombe cassera la \emph{box}, cela donnera une occasion au joueur de prendre un point ou un bonus.\\
Ensuite, s'il ne peut pas poser une bombe, il fera un mouvement vers une case adjacente parmis ceux qui sont possible. Le choix de la case se fera dans l'ordre de préferantielle suivant :
 \begin{itemize}
	\item une case ne lui exposant à aucun risque de mort et qu'un point ou un bonus soit disponible sur celui-ci.
	\item une case ne lui exposant à aucun risque de mort.
	\item une case lui exposant à un risque éventuelle de mort.
\end{itemize}
...
\begin{itemize}
	...
	\item \texttt{Player007Zorro} de Nizar Bousselmi. Aucun problème rencontré avec ce joueur.
	\item \texttt{Player010IA} de Xavier Nguyen. Aucun problème rencontré avec ce joueur.
\end{itemize}
\section{Extensions}
