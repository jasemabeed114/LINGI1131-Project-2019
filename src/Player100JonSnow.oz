...
in
    ...
    fun{SetMapValWithValue Map X Y Value}
        fun{Modif L N}
            case L of nil then nil
            [] H|T then
                if N == 1 then 
    ...
    in
        ...
        [] H|T then
            if Y == 1 then
                ...
                H|{SetMapValWithValue T X Y-1 Value}
            end
        end
    end
        ...
        in
            ...
            in
                ...
                if Tmp2 == 0 then % No BonusMove
                    TheMove
                in
                    TheMove = {BFS Map SafeMove}
                    case TheMove of null then % Should not happen
                        Rand2 = ({OS.rand} mod Tmp) + 1
                        Action = move({Nth SafeMove Rand2})
                    [] nil then  % Should not happen
                        Rand2 = ({OS.rand} mod Tmp) + 1
                        Action = move({Nth SafeMove Rand2}) 
                    else % Good move
                        Action = move(TheMove)
    ...
    fun{BFS Map SafePositions}
        fun{Loop Map Queue}
            if {IsDet Queue} then
                case Queue of (pt(x:X y:Y)#L)|T then
                    Chec1 Chec2 Chec3 Chec4
                    Map1 Map2 Map3 Map4
                    BoundingVariable
                in
                    if X+1 < Input.nbColumn+1 then
                        Chec1 = {GetMapVal Map X+1 Y}
                    else
                        Chec1 = 1
                    end
                    if X-1 > 0 then
                        Chec2 = {GetMapVal Map X-1 Y}
                    else
                        Chec2 = 1
                    end
                    if Y+1 < Input.nbRow+1 then
                        Chec3 = {GetMapVal Map X Y+1}
                    else
                        Chec3 = 1
                    end
                    if Y-1 > 0 then
                        Chec4 = {GetMapVal Map X Y-1}
                    else
                        Chec4 = 1
                    end

                    if Chec1 == 0 orelse Chec1 == 4 then % floor tile
                        {Send Port pt(x:X+1 y:Y)#(L)}
                        Map1 = {SetMapValWithValue Map X+1 Y 1}
                    elseif Chec1 == 1 then % wall
                        Map1 = Map
                    else % perfect
                        Map1 = Map
                        BoundingVariable = true
                    end
                    if Chec2 == 0 orelse Chec2 == 4 then % floor tile
                        {Send Port pt(x:X-1 y:Y)#(L)}
                        Map2 = {SetMapValWithValue Map1 X-1 Y 1}
                    elseif Chec2 == 1 then % wall
                        Map2 = Map1
                    else % perfect
                        Map2 = Map1
                        BoundingVariable = true
                    end
                    if Chec3 == 0 orelse Chec3 == 4 then % floor tile
                        {Send Port pt(x:X y:Y+1)#(L)}
                        Map3 = {SetMapValWithValue Map2 X Y+1 1}
                    elseif Chec3 == 1 then % wall
                        Map3 = Map2
                    else % perfect
                        Map3 = Map2
                        BoundingVariable = true
                    end
                    if Chec4 == 0 orelse Chec4 == 4 then % floor tile
                        {Send Port pt(x:X y:Y-1)#(L)}
                        Map4 = {SetMapValWithValue Map3 X Y-1 1}
                    elseif Chec4 == 1 then % wall
                        Map4 = Map3
                    else % perfect
                        Map4 = Map3
                        BoundingVariable = true
                    end
                    if {IsDet BoundingVariable} then
                        %{Delay 4000}
                        L % return the list of moves until there
                    else
                        {Loop Map4 T}
                    end
                else 
        ...
        proc{Init Positions}
            case Positions of nil then skip
            [] H|T then
                {Send Port H#H}{Init T}
            end
        end
        Port Stream
    in
